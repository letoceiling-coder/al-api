...
{
    /**
     * Получение списка домов с участками с фильтрами
     * 
    ...
    public function index(Request $request)
    {
        if (config('trendagent.data_source') === 'db') {
            return app(\App\Http\Controllers\TrendAgent\Db\HousesDbController::class)->index($request);
        }
        ...
        try {
            ...
            $apiData = $apiAuth->getHousesSearch($apiParams);
            ...
            Log::error('Ошибка получения списка домов', [
    ...
    {
        if (config('trendagent.data_source') === 'db') {
            return app(\App\Http\Controllers\TrendAgent\Db\HousesDbController::class)->show($request, $id);
        }
        ...
        try {
            ...
            Log::error('Ошибка получения детальной информации о доме', [
                'id' => $id,
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
