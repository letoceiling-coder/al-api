...
> **Примечание:** План объединяет предложения из Ответа 1 и Ответа 2
...
#### 1.4 Улучшение аутентификации (из Ответа 2) ⭐⭐

**Цель:** Добавить OAuth2/JWT для расширенной безопасности

**Задачи:**
- [ ] Установить `laravel/passport` или `tymon/jwt-auth`
- [ ] Реализовать OAuth2 с JWT токенами
- [ ] Добавить refresh tokens
- [ ] Реализовать scopes для разграничения доступа:
  - `ai:read` - только чтение
  - `ai:write` - создание запросов
  - `analytics:read` - доступ к аналитике
  - `keys:manage` - управление ключами
- [ ] Сохранить обратную совместимость с Sanctum
- [ ] Обновить документацию

**Преимущества:**
- ✅ Безопасные токены с автоматическим обновлением
- ✅ Лучший контроль доступа по ролям
...
### Приоритет 2: Важно для UX и масштабирования (2-3 недели)
...
**Цель:** Добавить больше информации в ответы (из Ответа 1 и 2)

**Задачи:**
- [ ] Добавить `model_version` в ответ
- [ ] Добавить `latency_breakdown` (время на каждом этапе)
- [ ] Добавить `request_trace` для отладки (из Ответа 2)
...
#### 2.4 Асинхронная обработка запросов (из Ответа 2) ⭐⭐

**Цель:** Поддержка тяжелых задач через очереди

**Задачи:**
- [ ] Создать endpoint `POST /api/v1/ai/process/async`
- [ ] Реализовать очередь для обработки (Laravel Queue)
- [ ] Создать endpoint `GET /api/v1/ai/process/{job_id}/status`
- [ ] Создать endpoint `GET /api/v1/ai/process/{job_id}/result`
- [ ] Реализовать webhook уведомления (опционально)
- [ ] Добавить polling механизм
- [ ] Обновить документацию

**Преимущества:**
- ✅ Не блокирует клиента на долгие запросы
- ✅ Возможность обрабатывать большие файлы
- ✅ Масштабируемость через очереди
- ✅ Webhook уведомления о готовности

**Пример использования:**
```bash
# 1. Создать асинхронный запрос
POST /api/v1/ai/process/async
{
  "provider": "gemini",
  "model": "gemini-1.5-pro",
  "prompt": "...",
...
# 2. Проверить статус
GET /api/v1/ai/process/550e8400-.../status

# 3. Получить результат
GET /api/v1/ai/process/550e8400-.../result
```
...
#### 2.5 Управление сессиями запросов (из Ответа 2) ⭐
...
- [ ] Добавить фильтрацию (по статусу, дате, модели)
- [ ] Добавить пагинацию
- [ ] Показывать детали запроса (токены, логи, статус)

**Оценка:** 2-3 дня

---

#### 2.6 Дополнительные параметры моделей (из Ответа 2) ⭐
...
#### 5.4 Grafana-friendly endpoints (из Ответа 2) ⭐
...
## 📅 Roadmap реализации (объединенный план)

### Фаза 1: Фундамент (Неделя 1-2)
- ✅ OpenAPI спецификация
- ✅ Версионирование API
- ✅ Улучшение ошибок (RFC 7807 + trace_id)
- ✅ OAuth2/JWT аутентификация (опционально)

**Результат:** API готов к интеграции, есть спецификация, улучшенная безопасность

### Фаза 2: UX и масштабирование (Неделя 3-5)
- ✅ Streaming ответы
- ✅ Multipart upload
- ✅ Расширенные метаданные
- ✅ Асинхронная обработка (из Ответа 2)
- ✅ Управление сессиями (из Ответа 2)

**Результат:** Улучшенный UX, поддержка больших файлов, масштабируемость

### Фаза 3: Инструменты разработчика (Неделя 6)
- ✅ Postman коллекция
- ✅ Sandbox среда
- ✅ Endpoint лимитов
- ✅ Дополнительные параметры моделей (из Ответа 2)

**Результат:** Удобные инструменты для разработчиков

### Фаза 4: Интернационализация (Неделя 7)
- ✅ Английская документация

**Результат:** Поддержка международных пользователей

### Фаза 5: Расширенная аналитика (Неделя 8-9)
- ✅ Прогнозирование
- ✅ Агрегация по моделям
- ✅ Учет по ключам
- ✅ Grafana-friendly endpoints (из Ответа 2)
- ✅ Пагинация и фильтрация (из Ответа 2)
...
| OAuth2/JWT | ⭐⭐ | 3-4 дня | Высокая |
| Асинхронная обработка | ⭐⭐ | 4-5 дней | Высокая |
| Управление сессиями | ⭐ | 2-3 дня | Средняя |
| Доп. параметры моделей | ⭐ | 1-2 дня | Низкая |
| Grafana endpoints | ⭐ | 2-3 дня | Средняя |
| Пагинация/фильтрация | ⭐ | 2-3 дня | Средняя |

**Общее время:** 8-10 недель (при работе 1 разработчика)

**С учетом MVP (только критичные):** 5-6 дней
